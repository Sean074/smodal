"""SIMO EMA page logic — called by pages/4_SIMO.py driver."""
...
def render() -> None:
    ...
    if uploaded_file is not None and st.session_state.get("simo_file_name") != uploaded_file.name:
        ...
        if err:
            ...
            for k in [
                "si_H_mat", "si_freqs", "si_cmif", "si_stability_table", "modal_results",
    ...
    with ctrl_col:
        ...
        frf_method = st.radio(
            "FRF method", ["Welch", "Single FFT"], horizontal=True, key="si_frf_method"
        )

        n_seg, ovlp_pct, welch_win = 8, 50, "hann"
        if frf_method == "Welch":
            n_seg = st.number_input(
                "Segments", min_value=2, max_value=50, value=8, step=1, key="si_segments"
            )
            ovlp_pct = st.slider(
                "Overlap (%)", min_value=0, max_value=90, value=50, step=5, key="si_overlap"
            )
        ...
        frf_est = st.radio(
            "FRF estimator", ["H1", "H2", "Hv"], horizontal=True, key="si_frf_est"
        )
        ...
        coh_red, coh_yellow = coherence_gate_warnings(
            st.session_state.get("si_spectral_channels"),
            st.session_state.get("si_spectral_freqs"),
            st.session_state.get("si_frf_method_used"),
            f_min,
            f_max,
        )

        max_order = st.slider(
            "Max model order", min_value=4, max_value=100, value=40, step=2, key="si_max_order"
        )
        ...
        with st.expander("Stability thresholds"):
            df_thr = (
                st.number_input(
                    "Δf threshold (%)", value=1.0, step=0.5, min_value=0.1, key="si_df_thr"
                )
                / 100.0
            )
            dd_thr = (
                st.number_input(
                    "Δξ threshold (%)", value=5.0, step=1.0, min_value=0.1, key="si_dd_thr"
                )
                / 100.0
            )
            mac_thr = st.slider(
                "MAC threshold", min_value=0.5, max_value=1.0, value=0.95, step=0.01,
                key="si_mac_thr",
            )

        build_btn = st.button(
            "Build Stability Diagram", type="primary", use_container_width=True, key="si_build"
        )
        ...
        n_modes = st.number_input(
            "Number of modes", min_value=1, max_value=20, value=auto_n, step=1, key="si_n_modes"
        )
        ...
        extract_btn = st.button(
            "Extract Mode Shapes", type="secondary", use_container_width=True, key="si_extract"
        )
    ...
    if build_btn:
        ...
        st.session_state.update(
            {
                "si_freqs": freqs,
                "si_stability_table": table,
                "si_cmif": cmif_vals,
                "si_H_mat": H_mat,
                "si_freqs_band": f_band,
                "si_H_mat_band": H_band,
                "si_sel_outputs": sel_outputs,
                "si_frf_est_used": frf_est,
                "si_spectral_channels": all_res,
                "si_spectral_freqs": _si_spectral_freqs,
                "si_frf_method_used": _si_frf_method_used,
            }
        )
    ...
    with chart_col:
        ...
        with tab_cmif:
            if cmif_vals is None or freqs_chart is None:
                ...
                for f_lo, f_hi in coh_yellow:
                    fig.add_vrect(
                        x0=f_lo, x1=f_hi, fillcolor="rgba(255,200,0,0.15)", layer="below", line_width=0
                    )
                for f_lo, f_hi in coh_red:
                    fig.add_vrect(
                        x0=f_lo, x1=f_hi, fillcolor="rgba(220,50,50,0.20)", layer="below", line_width=0
                    )
        ...
        with tab_shapes:
            if modal_res is None:
                ...
                show_modal = st.checkbox(
                    "Show individual modal contributions", value=False, key="si_show_modal"
                )
        ...
        # ── Spectral ─────────────────────────────────────────────────────────────
        with tab_spectral:
            spec_chs = st.session_state.get("si_spectral_channels")
            spec_freqs = st.session_state.get("si_spectral_freqs")
            method_used = st.session_state.get("si_frf_method_used")
